...
with beam.Pipeline('DirectRunner', options=opts) as p:
    ...
    query_results = p | beam.io.Read(beam.io.BigQuerySource(query='SELECT report_year, agency_code,rapes,rapes_percapita FROM crime_rates.crime_rapes limit 10'))

    # write PCollection to a log file
    query_results | 'Write to File 1' >> WriteToText('rapes_query.txt')

    #apply Pardo on the Pcollection
    state_pcoll = query_results | 'Create State abb' >> beam.ParDo(StateName())
    
    # write PCollection to a file
    state_pcoll | 'Write to File 2' >> WriteToText('output_rapes.txt')
    
    qualified_takes_table_name = 'han97jiayan:crime_rates.rapes_transfrom_simple'
